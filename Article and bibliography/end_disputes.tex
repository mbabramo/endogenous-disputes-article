...
\caption{Probability of liability curves for possible court signals as a function of defendant's relative precautio level. Successively higher curves reflect successively stronger signals of precaution power. In-between points (not used in simulation) are cubic interpolations.}
  \label{fig:probit}
\end{figure}
...
The court's task is to compare the marginal cost of precaution with the marginal benefit.
...
\]
The verdict is then drawn $V\sim \mathrm{Bernoulli}(w(c,k))$. Unless otherwise noted, we set $\lambda=1$ and use the distributed-threshold rule with $s=0.25$. Alternative $\lambda$ values, and in robustness checks the deterministic rule, are also examined.
...
For each $(c,k)$ pair, let $w(c,k)\in[0,1]$ denote the court’s decision weight (Appendix~A). Under the deterministic threshold, $w(c,k)=\mathbf{1}\{R_\star(c,k)>\lambda\}$; under the probit rule, $w(c,k)=\Phi\!\bigl((R_\star(c,k)-\lambda)/s\bigr)$. The probability of a liability verdict in the collapsed tree is
\begin{align*}
\Pr(V=1 \mid \theta,k)
