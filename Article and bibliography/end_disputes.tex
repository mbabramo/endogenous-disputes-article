...
It helps to briefly situate CFR. Exact sequence-form solvers (such as the well-known von Stengel–van den Elzen–Talman method) compute exact perfect equilibria on moderate-sized trees with mathematical precision. We use those where they fit, both for validation and for small benchmark specifications. But endogenous disputes with filing, answering, simultaneous offers, quit/default options, fee rules, risk attitudes, and trial—all under two-sided private information—generate trees  larger than exact methods comfortably handle. CFR is the scalable alternative: it trades proofs of exactness for speed and reach, while providing clear diagnostics of proximity to equilibrium.
...
Figure \ref{fig:liability_threshold} shows somewhat larger movements than the other interventions, though most of the welfare action still occurs in corner regimes. In our baseline calibration, the defendant-favorable, highest threshold (a benefit-cost threshold of 2.0) appears to minimize the sum of precaution, injury, and litigation costs. At this level, the defendant simply does not engage in precaution and, except with very low litigation costs, the plaintiff does not bother to sue. This does not mean that zero precaution is achieved; indeed, we will see later that the social optimum does involve positive precaution. But it does illustrate the possibility that from a pure efficiency perspective, given the costs and practical limitations of litigation, it sometimes may be more efficient for society to bear the costs than to allow litigation. Still, this point should not be overemphasized. The total social welfare with the very high liability threshold exhibits losses only six percentage points lower than the normal liability threshold of $1$. The middle range of thresholds ($0.8$, $1.0$, and $1.2$) all produce relatively similar results, with higher levels slightly reducing social cost.
...
Figure \ref{fig:dm_fee} shows that these two levers mostly operate on different, weakly connected margins, so their joint effect is close to additive and small. "Low D" and "High D" correspond to damages multipliers of 0.5 and 2, respectively. "Low F" and "High F" correspond to fee shifting multipliers of 0 and 2, respectively. The damages multiplier works, if at all, by altering precaution incentives and case selection; fee shifting reassigns relatively modest stage costs and nudges bargaining posture. Because transfers do not impact welfare and the per-stage costs are small relative to normalized harm, increasing the fee-shifting multiple barely changes real resource use. The relatively modest effects indicate that, controlling for the level of damages multiplier, double fee shifting is very modestly beneficial (though there is no effect with the low damages multiplier at the highest cost level). At either level of fee shifting, switching from the low to the higher damages multiplier is modestly harmful, at least at baseline or low litigation costs. But high damages multipliers greatly outperform low damages multipliers at the highest cost levels, consistent with Figure \ref{fig:damages_multiplier}.
...
Figure \ref{fig:fee_liability} tells a similar story: Overall, the social costs level is relatively static, with higher liability thresholds producing modest savings controlling for fee-shifting level. "Low L" refers to a liability threshold of 0.8, and "High L" refers to a threshold of 1.2. Figure \ref{fig:fee_liability} does feature the panel that has the strongest performance at baseline costs of any intervention seen so far: the combination of high fee-shifting with a high liability level. But this is optimal not because it leads to settlements, but because plaintiffs are hesitant to file in truly not liable cases and even in cases in which the defendant is truly liable. As before, relatively low precaution costs in this model appear to minimize the total social cost.
