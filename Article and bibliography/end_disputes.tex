...
After the plaintiff files and the defendant answers, we insert—before any settlement proposals—the parties’ contingent exit choices: the plaintiff may elect to abandon if bargaining later fails, and the defendant may elect to default if bargaining later fails. This placement is counterintuitive but deliberate. Modeling these “quit if no settlement” decisions \emph{after} every possible offer profile would require intersecting abandon/default with the settlement subgame, exponentially multiplying information sets without changing what the parties can do or the distribution of outcomes. By recording the quit choices immediately after the file/answer node (and keeping them unobserved until they matter), we preserve most of the strategic richness while dramatically simplifying the tree.

Bargaining then proceeds exactly as in the models of Abramowicz (2025a) \cite{abramowicz2025a} and Abramowicz (2025b) \cite{abramowicz2025b}. The plaintiff states a minimum monetary demand (labeled “P Offer” in the tree diagram) and the defendant announces the most it is willing to pay (“D Offer”). Although the diagram prints P Offer before D Offer, this ordering is irrelevant. Because the defendant does not receive information about the plaintiff offer before announcing its own amount, the decisions are effectively simultaneous. If the offers meet or cross (the defendant's offer is at least as great as the plaintiff's), the case settles at the midpoint of the two numbers and litigation ends. The parties may be conceived as negotiating over the unmultiplied damages, but the damages multiplier then applies to any settlement value. The defendant pays to the plaintiff the amount of the settlement. In addition, each party pays a cost for filing or answering, as well as a cost of bargaining, but then saves the cost of trial. In the baseline condition, the stakes are normalized to $1$ and the cost of each phase (filing or answering, bargaining, and trial) is $0.10$ per player. Thus, if a case goes to trial, each party pays $0.30$.
...
Because litigation costs make litigation a general-sum game, CFR does not come with the same global convergence guarantees it enjoys in zero-sum settings. We therefore certify output in two ways. First, we compute \emph{exploitability}: given one party’s strategy, we solve that party’s best response and report the gain from deviating. In the specifications we present, those gains are tiny relative to case stakes. Second, where an exact solver fits (smaller trees that omit some modules), we check that CFR reproduces the exact equilibrium’s aggregate predictions. 
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth, clip, trim=0 0 0 4.7cm]{../Figures/PREC008 Risk Neutral CM 1 FSM 0 DM 1 LT 1 UPC 1E-05 -costbreakdownlight.pdf}
...
A natural question from scrutiny of Figure \ref{fig:baseline_costs} is why litigation does not occur more often. In the vast majority of cases in which harm occurs, no litigation occurs as a result. There are a few cases in which the plaintiff does file a claim, and in this simulation, the defendant answers and bargaining ensues. In a relatively small proportion of those cases, a trial occurs. It might seem that litigation would be considerably more common. After all, if one examines Figure \ref{fig:probit}, any given level of relative precaution can correspond to eight different precaution power curves, and the probability of liability can vary greatly depending on what the actual precaution power was. 
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{../Figures/PREC008 Risk Neutral CM 1 FSM 0 DM 1 LT 1 UPC 1E-05 -winratebypdsignallight.pdf}
