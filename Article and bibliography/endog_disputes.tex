...
Answering this question, however, requires more than a model of the litigation process itself, even with a better characterization of that process than before. To address implications of design for overall welfare, we cannot look at litigation outcome variables--such as settlement rates or accuracy--in isolation. Rather, we must assess the ramifications of changes in the litigation system for primary behavior. As Shavell (1997) \cite{shavell1997} recognized, litigation is rife with externalities. The mere expectation of litigation may affect third parties through deterrence (and perhaps overdeterrence), and litigants impose externalities on one another in litigation. There can thus be too much litigation or too little, but procedural mechanisms can affect the calibration of such incentives. To fully tally the consequence of different choices of procedural mechanisms, we must examine not only the effects of such mechanisms on any given suit, but also on the legal system as a whole. Even if we cannot be sure of the precise consequences of procedural choices for litigation costs, we may be able to gain appreciation of the extent to which the hydraulic adjustments of the litigation system constrain gains or losses from good or poor choices. 

Given the challenges of mathematical modeling, the theoretical literature has for the most part avoided the challenge of simultaneously modeling both the generation of disputes and the litigation process that resolves them. Models of the tort process often take the litigation system as a given, for example by assuming a fixed cost of litigation cost without fully assessing the dynamics of trial. Shavell (1980) \cite{shavell1980}'s canonical comparison of strict liability versus negligence assumes the absence of litigation costs, and even many more modern models, such as Baumann and Rasch (2024) \cite{baumannrasch}'s model of product liability, reasonably adopt a similar assumption. Meanwhile, models of the litigation process itself, such as models of fee shifting, may begin from the assumption that a dispute has occurred, thus ignoring the possibility that a different litigation system might have avoided an accident or other source of controversy. Argenton and Wang (2023) \cite{argentonwang} is just one of many recent examples that assume the existence of a dispute because doing so allows undistracted focus on the litigation process.

Some modelers have commented on the design of the litigation process using models in which the generation of disputes is endogenous, but have been forced to accept great simplifications about the process of adjudication itself. P'ng (1983) \cite{png}'s model is one of the first in which injurers vary in the cost of care and choose care endogenously, and the entire game is simplified into a extensive form game tree with only 15 nodes. His model demonstrates that the litigation process shapes incentives for care but leaves open many interesting questions of institutional design. Polinsky and Rubinfeld (1988) \cite{polinskyrubinfeld1988} show that optimal liability may require an increase or decrease in damages to balance the goals of optimizing care and minimizing litigation expenditures, but they unrealistically assume no judicial error. Spier (1994) \cite{spier} provides a sophisticated model of settlement bargaining and explores trade-offs with respect to whether damages awards should be finely tuned to the level of harm, but she too assumes no errors in measuring liability. Meanwhile, Polinsky and Shavell (2014) \cite{polinskyshavell2014} allow for litigants to face fixed and variable costs of litigation depending on the level of damages, but courts again adjudicate without error. Yet the possibility of judicial error may be critical to assessing the efficiency of procedural mechanisms. If, for example, treble damages are assessed sometimes against a not truly liable party, that might be problematic even if the treble damages help mitigate the danger of underdeterrence. 

One scholar has furnished a model with endogenous disputes and imperfect adjudication, Keith Hylton. Over three papers, Hylton develops what he calls a "cradle to grave" model of litigation. In his models, potential injurers must decide how much care to take, and these decisions determine how many accidents occur, which in turn result in lawsuits in which there might or might not be fee shifting. In Hylton (1990) \cite{hylton1990}, Hylton (1993) \cite{hylton1993} and Hylton (2002) \cite{hylton2002}, the potential tortfeasor draws a random cost of care and then compares it to the benefit in expected liability savings from compliance, taking into account the risk of type-1 and type-2 errors. The first article does not explicitly model settlement, while the second assumes that cases settle whenever there is a zone of agreement (that is, the plaintiff's minimum acceptable settlement is not greater than the defendant's maximum offer). The third article features a Bayesian settlement model in which plaintiffs play mixed strategies in determining whether to accept defendants' offers.  

To accomplish the remarkable feat of finding equilibria across both care and litigation decisions, Hylton necessarily must make some sacrifices in the realism of the model of litigation. While these models admit of judicial error, the risk of error is no greater when the level of care is close to the legal standard than when it is far from it. In addition, the strategic interactions between plaintiffs and defendants are stylized in order to yield tractable equilibria: bargaining is reduced to simple settlement zones in the second paper, and the informational structure is entirely one sided in the third of the trilogy, thus avoiding the methodological complexity of two-sided asymmetric information. In reality, though defendants may have more knowledge than plaintiffs of their own care decisions, that advantage is mitigated by discovery. Thus, parties are likely both informed about the probability of liability to some imperfect degree, with differences in assessments resulting from lawyers making varied predictions based on largely the same evidence.

Recent literature, meanwhile, has made progress in modeling two-sided asymmetric information, though without Hylton's innovation of endogenous disputes. Earlier models, such as Bebchuk (1984) \cite{bebchuk84} and Daughety and Reinganum (1994) \cite{daughetyreinganum1994}, allowed for two-sided asymmetric information by granting each party information on different quantities, for example with one party knowing the level of damages and the other party knowing the level of liability. But Friedman and Wittman (2007) \cite{friedmanwittman} and Klerman, Lee and Liu (2018) \cite{klermanleeliu} model the situation in which each party has independent private information about the same issue, such as the level of damages. Dari-Mattiacci and Saraceno (2020) \cite{darimatiaccisaraceno} manage to incorporate an analysis of the effect of fee shifting on such information. Following Friedman and Wittman, Dari-Mattiacci and Saraceno assume that each party receives an independent signal of the level of damages and that the total damages is equal to the sum of the signals.

As Abramowicz (2025a) \cite{abramowicz2025a} points out, however, tractability required Dari-Mattiacci and Saraceno to make critical assumptions, such as relatively low litigation costs, that every potential lawsuit is always contested, and that a linkage exists between the degree of information asymmetry and the true merits of the case. Rather than complicate an already intricate mathematical model, Abramowicz (2025a) switches to using a computational game theory algorithm to study similar models in which the outcome of litigation depends on the sum of the parties' signals. The linear programming algorithm, developed by von Stengel, van den Elzen, and Talman (2002) \cite{vonstengelvandenelzentalman}, identifies exact perfect Bayesian Nash equilibria in extensive form game trees. Abramowicz (2025a) applies this algorithm to extensive form games of over 16,000 nodes, allowing incorporation and variation of a number of game features that could not easily be jointly modeled mathematically. These features include uncertainty about liability rather than damages, risk aversion, and options not to sue or defend, both at the outset of litigation and after the failure of negotiation.

The results strongly suggest that generalizations based on simpler models may not survive more robust specification. For example, Dari-Mattiacci and Saraceno found that the English (loser pays) rule would be relatively advantageous in a jurisdiction with relatively low litigation costs and the American rule would be advantageous in a jurisdiction with relatively high litigation costs. None of the major computational specifications supported both of these results. Of course, computational results will not necessarily be robust either. Abramowicz (2025b) \cite{abramowicz2025b} modifies the earlier model, featuring game trees of over 45,000 nodes that enable correlated instead of independent signals. That is, each party receives a noisy estimate of the true value of the litigation. In this setting, fee shifting can have complicated strategic effects. On one hand, it may lead parties with weak cases to give up or settle on unfavorable terms. On the other, especially when costs are relatively high, it may encourage a party to bluff in the hope that the other party will quit first. 

A significant limitation of these computational analyses is that they feature disputes that are exogenously generated, with an even chance of a case being one in which the defendant is truly liable and an equal chance of being one in which the defendant is not truly liable. Truly liable cases are assumed to have stronger evidence for the plaintiff than not truly liable cases, but the distribution of litigation strength given true liability is also set based on an arbitrary parameter. In essence, in these models, as in many litigation models, cases in effect fall from the sky. This leaves unresolved questions of whether the conclusions would be different if there were a different distribution of truly liable vs. not truly liable cases. More critically, it elides the interaction effects between the litigation process and the generation of disputes. The computational approach to date has modeled how fee-shifting affects cost and accuracy in various environments, but it has not previously combined these considerations, as Hylton does, in an effort to identify global optimality.

This article applies algorithmic game theory, albeit with a different algorithm that makes it feasible to feature much larger game trees, containing over 225,000 nodes. The greater size of these game trees makes it feasible to integrate potential tortfeasors' decisions about whether to take care, while still maintaining the richness of the bargaining framework of the earlier papers. It thus continues Hylton's project of modeling litigation cradle to grave, while allowing for two-sided asymmetric information. Each party's signals span multiple levels of litigation quality, and settlements can occur at each of multiple payment levels. The parties follow the Chatterjee and Samuelson (1983) \cite{chatterjeesamuelson} bargaining protocol, in which each party submits an offer and cases settle at the midpoint if the defendant's offer equals or exceeds the plaintiff's. Although the algorithm uses floating point numbers and finds approximate equilibria, unlike the exact algorithm operating on rational numbers of the von Stengel et al. algorithm, these equilibria are measurably quite close to perfect Bayesian Nash equilibria. 

This article's analysis is not limited to fee shifting. Recognizing Craswell (1999) \cite{craswell}'s point that a variety of different types of procedural mechanisms might help achieve optimality, this article also considers two other procedural variations: damages multiples and alterations in the liability threshold. Some simulations intersect multiple such variations, to enable consideration of, for example, a regime in which it is difficult to establish liability but liability once established is heavy. With each simulation, it is possible to observe the sum of precaution, injury, and litigation costs, and differences across simulations can thus be at least tentatively attributed to differences in parameters and, in particular, differences in the procedural mechanisms used. 

Though it is difficult to encapsulate all of the article's findings in a single conclusion, a central point emerges: When lawsuits are generated endogenously based on the potential defendant's level of care, the adjustments in care to the legal environment will generally prevent extreme variation in total costs. If liability levels are high, for example, precaution costs will be relatively high, but injury and litigation will be relatively low. Environments that are especially defendant-favorable, particularly with high liability thresholds, perform relatively well, though the differences in total social cost are not large. That does not mean that the procedural mechanisms are entirely irrelevant, and indeed performance is sufficiently below that of a hypothetical optimal legal system to encourage continued search for better mechanisms. But the effects of even aggressive procedural mechanisms such as damages multiples are much more muted than one might think. 

The results that follow point in a consistent direction. When disputes are generated endogenously and litigants bargain under two‑sided private information, the civil justice system tends to re‑equilibrate in ways that blunt the force of familiar procedural levers. Increasing damages multipliers does shift private incentives toward greater precaution, but the induced changes in settlement posture and activity choice offset much of the raw leverage of higher stakes; welfare moves materially only at the extremes. Fee shifting is still less potent because it mainly reallocates relatively small stage costs, leaving primary behavior and total social cost largely unchanged. Adjusting the liability threshold matters more, chiefly by screening claims ex ante and thereby curbing procedural expenditures, yet even here the differences in total cost are modest over a wide range, and strict liability performs poorly once litigation is costly, though in part because of how the model sequences plaintiff and defendant decisions. The central lesson is not that procedure is irrelevant, but that in a system where care, filing, bargaining, quitting, and adjudication respond to one another, policy levers often move composition more than totals. This both cautions against relying on any single instrument and motivates the search for designs that shift multiple margins at once or operate where equilibration is weakest.

\section{An Extensive Form Endogenous Litigation Game}

The approach that would be most consistent with Hylton's and indeed the broader literature on the economics of accidents would be to allow the potential defendant to choose a level of care, aware of the implications of that level for the probability that an accident occurs. Though perhaps plausible in some cases, this approach would have a serious drawback: When a case arises, the defendant would necessarily know how much care was exerted and therefore the quality of the lawsuit. In reality, both plaintiffs and defendants will often be uncertain about the outcome of litigation. Moreover, after initial accumulation and sharing of information, whether through formal discovery or otherwise, it is not obvious that the defendant's knowledge in every case will necessarily be better than the plaintiff's. Such an asymmetry might manifest in many cases, but it would be preferable if that possibility could be explored merely as one setting of the model. A neutral default setting would be one in which each side has equal insight into litigation quality. 

A modeling approach addressing this concern emerges from the recognition that in many settings, both parties can observe what level of precaution the injurer actually took (for example, whether a machine guard was installed or which safety protocol was followed), and may even have strong information about the cost of such precautions. Meanwhile, the parties also may know what precautions the injurer forewent and how much such additional precaution might have cost. Nonetheless, there might still be uncertainty concerning how \emph{effective} the marginal precaution not taken would have been at reducing risk and whether such a risk reduction would have been cost-justified. Effectiveness depends on technological fit, local conditions, and other hidden factors that are not fully verifiable or knowable ex ante or perhaps even after an accident occurs. Such questions can be particularly challenging because they are counterfactual. Disputes in tort cases will thus often concern whether the defendant should have taken a more cautious path than the defendant actually took. To be sure, there may sometimes be debates as well as to what the defendant actually did and as to what a hypothetical precaution would have cost, but even with computation, analytic tractability makes desirable focus on a single source of uncertainty. For the purpose of the model in this article, the fount of uncertainty will thus concern the level of \emph{precaution power}, that is the extent to which further expenditures on precaution would have reduced the probability of an accident. Each party knows the \emph{precaution level} but receives only a signal of precaution power. 

This section thus presents a single extensive-form model in which precaution and litigation are determined jointly in equilibrium under a negligence rule with imperfect adjudication and two-sided private information. Chance determines an underlying state that governs how effective precaution is at reducing accident risk. The defendant chooses a discrete relative precaution level before any accident occurs. The hidden precaution power, chosen precaution level and chance then jointly determine whether the defendant causes an accident.

Figure \ref{fig:precaution} illustrates, using the baseline calibration that will be our focus, the interaction between precaution power levels and the defendant's relative precaution level in our model. The vertical axis represents accident probability, with a maximum accident probability of $1.0 \times 10^{-4}$. The horizontal axis specifies the relative precaution level chosen. The curve is defined continuously, but for computational tractability, the defendant is required to choose from a collection of discrete points. The eight discrete points permitted are marked on each precaution power curve. Different conventions may be used to measure the marginal benefit of reducing an accident at each of these points; the marginal benefit may be defined as the instantaneous reduction in slope or the difference from the next discrete level, and with the former, either assuming that the top level represents a safe harbor or not. In our baseline, the marginal benefit of precaution is measured next to the next discrete level chosen but in the case of the highest permissible level, relative to the hypothetical next discrete level visible at the far right of the graph. This choice is explained further in Appendix A.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{../Figures/precaution.pdf}
\caption{Accident probability as a function of precaution level, for different hidden precaution–power states. Successively lower curves reflect successively stronger precaution powers.}
  \label{fig:precaution}
\end{figure}

 If the defendant does not cause an accident, there is nonetheless some chance, unrelated to the level of precaution, that an accident is wrongly attributed to the defendant. Assuming the defendant does injury the plaintiff or such an injury is wrongly attributed to the defendant, the plaintiff decides whether to file suit, the defendant decides whether to answer or default, the parties engage in a one-shot simultaneous-offers bargaining stage (with acceptance if the defendant’s offer weakly exceeds the plaintiff’s demand). If there is no settlement the case proceeds to trial unless one party decides to quit upon settlement failure. 

At trial, the court applies a probabilistic liability test using its noisy signal of precaution power, together with the observed relative precaution level. In the baseline specification reported here, the court is modeled as if there is an official liability threshold of $1.0$, but individual judges differ in the exact threshold they individually apply. Some judges effectively require a somewhat higher benefit–cost ratio than $1.0$, others somewhat lower. As a result, for any given case, the chance of a liability verdict is the fraction of judges whose personal thresholds would be exceeded by the court’s estimate of the benefit of further precaution. This produces a smooth rise in the probability of liability as the expected benefit crosses the doctrinal threshold, as shown in Figure \ref{fig:probit}. The higher signals indicate greater precaution power, and so, controlling for relative precaution level, a higher signal implies a greater benefit that would have accrued from more precaution and thus a higher probability of liability. If instead all judges applied exactly the same cutoff of $1.0$ without variation, any point on Figure \ref{fig:probit} with a probability of liability greater than $0.5$ would result in liability with a probability of $1$, and any other point would result in liability with a probability of $0$. The distributed-threshold rule provides a more realistic interpretation that avoids such cliff effects while preserving the idea of a common doctrinal benchmark at $1.0$.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.8\textwidth]{../Figures/probit.pdf}
\caption{Liability-probability curves reflecting heterogeneous judicial preferences as a function of the defendant's relative precaution level. Successively higher curves reflect successively stronger signals of precaution power. In-between points (not used in simulation) are cubic interpolations.}
  \label{fig:probit}
\end{figure}

Given a finding of liability, the defendant is responsible for damages, which may be scaled by a multiplier to represent punitive or enhanced awards. Both parties also bear litigation costs, and fee-shifting rules determine whether those costs are shared or shifted. From the broader perspective of social welfare, we add up not only these litigation expenditures but also the underlying precaution costs borne by defendants and the accident losses that remain after precaution is chosen. In this way, welfare reflects the full set of resources used or wasted in the process—what defendants spend to prevent accidents, what is lost when accidents nevertheless occur, and what both sides spend to fight over liability in court.
...
An extensive-form game tree can fully capture the formal game summarized above for any particular set of parameter values. The game features two strategic players and Chance. The \emph{defendant} (potential injurer) chooses whether to engage in the underlying activity, and, if so, a discrete precaution level. (The choice whether to engage in the activity and thus receive the fruits of that activity has little role in the analysis in this paper, but is included both for analytic completeness and as a source for analysis in future work.) The \emph{plaintiff} (victim) decides whether to file, which demand to submit in simultaneous offers, and whether to abandon in the post-offer exit stage if bargaining fails. The defendant similarly decides whether to answer, which offer to submit, and if bargaining fails, whether to default in the post-offer exit stage. The \emph{court} is modeled as a chance node that observes a noisy liability signal at trial and applies (under our baseline settings) a probabilistic liability test. Near the level where marginal benefit equals cost, the probability of liability is close to even; further from that level, the probability approaches either $0$ or $1$.

In the baseline calibration used for analysis, the number of hidden precaution-power states is $8$, the number of discrete precaution levels is $8$, and each party’s liability signal also has $8$ levels. In addition, the number of possible settlement offers that the plaintiff and defendant, respectively, may make, is also $8$. In the miniaturized version of the game tree explored here, all of these eights become twos (for the hidden state, precaution levels, and the plaintiff/defendant signals and offers). Otherwise, parameters are set to the baseline values that we use in the simulation with the larger tree. 

Even the miniaturized version of the game is far too detailed to be legible if included in full here. We must zoom into particular portions of the game tree to allow for clearer explication of the game. The game proceeds in three broad stages. For each stage, we first present the technical structure of the game, and then discuss how the corresponding figure illustrates that structure. 

Figure \ref{fig:smalltree_beginning} provides the first stage in the miniature version of the game. The diagram shows the split of hidden states representing precaution power at the left, the branching for defendant and plaintiff signals just to the right of that, the defendant’s decision to engage in the activity and select a relative precaution level, and the accident node at the far right. Ellipses are used to indicate the vast portions of the miniaturized game tree that are omitted. 
...
The accident probabilities listed reflect the sum of the probability that the defendant causes injury to the plaintiff and that the defendant is wrongfully believed to have caused such injury. This nuance is omitted from the game tree because whether attribution is proper or wrongful has no effect whatsoever on game play. Though a more sophisticated model might explicitly model causation, that is not modeled here. Whatever the route to an injury relevant to the model, the legal attribution of causation to the defendant is assumed. The trial itself concerns only whether the defendant exercised standard care, not the issue of causation. Thus, after applying the proper formulas to identify the total accident probability, we can ignore the causation issue in finding perfect Bayesian Nash equilibria. Then, in the final accounting, we can break down the proportion of cases in which causal attribution is wrongful if desired, as will be apparent in diagrams reporting results from simulations. Note that the model does not reflect that the plaintiff may be injured by someone other than the defendant, because that has no bearing on the defendant's liability or on game play. For welfare accounting, in any event, only injuries physically caused by the defendant are included; injuries that may occur under wrongful attribution are excluded, since the welfare measure captures only the consequences of the defendant’s own activities.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{../Figures/smalltree_mid.pdf}
  \caption{Stage II: filing, answering, and a single round of simultaneous offers.}
  \label{fig:smalltree_mid}
\end{figure}

As illustrated in Figure \ref{fig:smalltree_mid}, if an accident occurs, the plaintiff decides whether to file suit after observing the plaintiff's private signal. If the plaintiff does file suit, the defendant then decides whether to answer (contest). Answering imposes a cost at baseline of $0.10$ and moves the case into bargaining; default is then not available until the post-offer exit stage described in Figure~\ref{fig:smalltree_end}.

Bargaining then proceeds as in Abramowicz (2025a, 2025b).  The parties make a single round of simultaneous monetary offers on finite grids: the plaintiff states a minimum demand (“P Offer”) and the defendant a maximum payment (“D Offer”). Although the diagram prints P Offer before D Offer, the choices are simultaneous—the defendant does not observe the plaintiff’s demand when setting its own offer. If the offers meet or cross, the case settles at the midpoint of the two amounts. The damages multiplier applies to this settlement amount; fee‑shifting does not. Each party pays filing/answering and bargaining costs but avoids trial. In the baseline, stakes are normalized to $1$ and the per‑party cost of filing (or answering), bargaining, and trial is $0.10$, so a case that goes to trial costs each side $0.30$ in stage costs.

If the offers do \emph{not} meet, the model then enters a post‑offer exit stage, illustrated in Figure \ref{fig:smalltree_end}. At that point, the plaintiff may choose to \emph{abandon} (terminating the case with no judgment) and the defendant may choose to \emph{default} (terminating with a damages judgment equal to the applicable multiplier). These exit decisions are simultaneous—implemented in the tree via sequential moves with information sets so neither party observes the other’s choice when deciding. If exactly one party quits, that choice is implemented. If both quit, a symmetric chance node determines which quit occurs first (so that the other party does not incur unnecessary costs). If neither party quits, the case proceeds to trial.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{../Figures/smalltree_end.pdf}
  \caption{Stage III: post-offer exit stage (abandon/default), symmetric both-quit chance node, and trial only if neither quits.}
  \label{fig:smalltree_end}
\end{figure}

At trial, the court observes a noisy signal of precaution power and applies the negligence test using the distributed‑threshold rule described above: the liability probability at a given precaution level and court signal equals the fraction of judges whose idiosyncratic thresholds would be exceeded by the court’s benefit–cost estimate. A damages judgment (scaled by the multiplier) and any fee‑shifting are then applied, and remaining stage costs are assessed.
...
It helps to briefly situate CFR. Exact sequence-form solvers, such as the von Stengel et al. (2002) \cite{vonstengelvandenelzentalman} method, compute exact perfect equilibria on moderate-sized trees using rational numbers. But endogenous disputes with filing, answering, simultaneous offers, quit/default options, fee rules, risk attitudes, and trial—all under two-sided private information—generate trees  larger than exact methods comfortably handle. We attempted to solve the baseline simulation represented in Figure \ref{fig:baseline_costs} using the von Stengel et al. method, and it had failed to find a solution within 24 hours. More concerningly, the time per pivoting step was increasing exponentially. This is consistent with the general observation by Fang and Havas (1997) \cite{fanghavas} that when applying linear programming algorithms that use rational numbers, the encoding length of intermediate entries can become ever larger, requiring the calculation of ever more challenging greatest common denominators and least common multiples. Yet unfortunately, the von Stengel et al. algorithm can prove unstable with floating point approximations, even on the simpler game trees of Abramowicz (2025a).

CFR is the scalable alternative: it trades proofs of exactness for speed and reach, while providing clear diagnostics of proximity to equilibrium. Two features make CFR especially apt for litigation models. First, it learns \emph{at information sets}. A party must commit to the same mixed action wherever it cannot tell histories apart (for example, “plaintiff with a given private signal before seeing any offer”). CFR updates precisely at that granularity, which is the unit that matters for sequential rationality. Second, the learning is \emph{counterfactual}. When the algorithm evaluates, say, how generous the defendant’s offer should have been, it holds fixed the opponent’s strategy and the chance moves that make this information set arise, and it asks how outcomes downstream would have changed had the defendant alone behaved differently. This isolates the value of each alternative in a way that respects imperfect information and the logic of Bayesian updating.

Our implementation uses \emph{full-tree CFR}, also called \emph{vanilla CFR}, not a sampling variant (though the code base also includes support for various sampling approaches).  Each iteration walks the entire game tree once, records the realized payoffs, computes counterfactual improvements at every information set, and then updates the mixed strategies. Three practical choices improve stability without changing the target notion of equilibrium. We begin with tiny “tremble” probabilities so all actions remain available early on; we lightly discount very old regret so the process is not hostage to early noise; and we track both the current strategy and its running average, reporting results from the latter, which is standard in this family of methods.

Because litigation costs make litigation a general-sum game, CFR does not come with the same global convergence guarantees it enjoys in zero-sum settings. We therefore certify output by computing \emph{exploitability}: given one party’s strategy, we solve that party’s best response and report the gain from deviating. In the specifications we present, those gains are tiny relative to case stakes. 

CFR’s strengths match the needs of this project. Each iteration is linear in the number of decision points, so scale comes from walking the tree, not from enumerating astronomical pure strategies. The method is modular: fee-shifting rules, damages multipliers, liability thresholds, risk preferences, wrongful-attribution channels, and quit/default options enter as plug-in payoffs without rewriting the algorithm. It naturally accommodates two-sided private information and simultaneous offers, and though it demands perfect recall, our game trees satisfy this requirement by construction.

Its limitations deserve equal candor. CFR produces highly accurate \emph{approximations}, not certificates; closeness is demonstrated by diagnostics rather than guaranteed a priori. Results depend on discretization choices for signals, offers, and precaution levels; we therefore report robustness to grid refinements and use exact solvers for small-tree benchmarks. Multiple equilibria are a feature, not a bug, in many litigation games; CFR will converge to one, not enumerate all. Finally, full-tree passes are computationally heavy—tractable at our scales, but still the main cost driver. The total number of processor hours used to compute the data used in this paper was 720, and so using still larger trees or more exact algorithms would have been quite costly in a practical sense.

In short, CFR lets us compute equilibrium behavior in a richly specified, cradle-to-grave litigation model. We use it to solve many different permutations of the full endogenous-disputes game; we use exact sequence-form solvers as spot checks on reduced versions of the same game; and we use exploitability and to verify that the solutions we report are both stable and economically meaningful. A more formal presentation appears in the appendix, which spells out the learning loop, the diagnostics, and the precise stopping rules we employ.

\section{Results}

This section reports the primary results. The goal is less to catalog every specification than to highlight the main comparative-statics patterns and welfare implications under the baseline and the three interventions. An Online Appendix available at \url{https://github.com/mbabramo/endogenous-disputes-article} contains over 50,000 files, with information on and diagrams reflecting 2,269 permutations of the endogenous litigation game, including many permutations that we do not have space to discuss here. The code is at \url{https://github.com/mbabramo/ACESim4/tree/endogenous-disputes}. (The vast majority of the approximately 80,000 lines of source is hand-written, but some of the more recent source code was prepared with the assistance of AI tools and then manually reviewed and edited.)

\subsection{Baseline}

The results of any given simulation can be summarized easily in a diagram that illustrates social welfare losses disaggregated among precaution costs, injury costs, and litigation costs. Figure \ref{fig:baseline_costs} provides this illustration at the baseline parameter values of the simulation (which are described above and also documented in Appendix A). The diagram is divided into two parts, for the cases in which no accident occurs (and thus no potential dispute arises) and those in which there is an actual or wrongly attributed accident (and thus a potential dispute). The left side of the diagram represents the vast majority of cases, in this case approximately 99.993\%. But for the vertical axis, representing costs, the values on the left side are several orders of magnitude lower than the values on the right side. The axis scales have been carefully selected to equalize a square unit of cost on both sides of the diagram. A horizontal unit on the left side represents a far higher proportion of cases than a horizontal unit on the right side, but a vertical unit on the left side represents a much lower level of costs than a vertical unit on the right side. The values chosen ensure that the product of a horizontal and a vertical unit are the same on both the right and left sides. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth, clip, trim=0 0 0 4.7cm]{../Figures/PREC010 Risk Neutral CM 1 FSM 0 DM 1 LT 1 UPC 1E-05 -costbreakdownlight.pdf}
  \caption{Social costs with baseline parameter values.}
  \label{fig:baseline_costs}
\end{figure}

The result is that the contributions to social welfare losses of very common but low precaution costs can be directly compared to the contributions to social welfare losses of injury costs and litigation costs, which may be accrued either in cases in which the defendant has not caused any injury the plaintiff may have suffered or those in which the defendant genuinely caused an injury to the plaintiff. That these costs are the exclusive components of social welfare is a standard assumption since at least Calabresi (1970) \cite{calabresi}. One can, of course, argue for a more sophisticated social welfare function. For example, it might take into account distributional consequences. In general, distributional consequences might mean that a unit of precaution is in a sense less costly than a unit of injury, since injuries are generally concentrated rather than diffuse. Determining how to weigh distributional consequences will be context-sensitive, however, and is beyond this article's scope. Another challenge to this simplistic social welfare accounting might arise from the study of procedural justice, as in Tyler (2003) \cite{tyler}. Perhaps litigation costs, particularly trial costs, promote feelings of procedural justice and increase legal compliance. Again, however, this is beyond our scope here.


In Figure \ref{fig:baseline_costs}, two different levels of precaution cost are visible on the left side of the diagram. This reflects that precaution power varies and different defendants will have received different signals of precaution power and will have made different decisions based on those signals. In this equilibrium, the defendant chose to exert two positive units of precaution whenever the defendant received one of the four lowest signals, and the defendant chose to exert four positive units the other half the time. (The diagram reflects this, but the precise numbers may be confirmed on the cross-tabs available in the Online Appendix.) It is not intuitively obvious why the defendant does not choose the intermediate value of three positive units of precaution with any positive probability for any signal value. But this is indeed an equilibrium--or so close to one that a player changing its strategy would not be able to improve its score by more than $2.65 \times 10^{-7}$ of the possible score range--establishing that no alternative strategy could be meaningfully superior, keeping the other party's fixed. Interestingly, it is common that identified equilibria are those in which a party's strategy has support over noncontiguous actions.

Precaution costs are still in principle illustrated on the right side of the diagram; that is, cases in which an injury occurred are not solely cases in which the potential defendant chose not to exercise precaution (though some might be). The precaution costs are just so much smaller than the injury costs in a case in which injury occurs that they are negligible and are not even visible on this automatically constructed diagram. Indeed, even when measured as total contributions to social welfare loss, precaution costs are, in both this diagram and those associated with the vast majority of other simulations, generally much lower than the total costs of injury. 

Defendants face two channels of incentive to raise precaution. First, more care lowers accident risk. In our calibration the marginal reduction in risk is front‑loaded—early units of care do most of the work—and a positive residual risk remains even at high care because accidents cannot be eliminated and a small wrongful‑attribution hazard persists. Second, more care lowers the chance of an adverse verdict and, through that, improves the defendant’s bargaining position. Because both parties and the court observe only noisy signals, the mapping from care to liability may involve uncertainty. Taken together, diminishing returns in accident prevention and noisy adjudication mean the marginal private benefit of very high care is often below its marginal cost, so equilibrium care typically stops short of the maximum.

A natural question from scrutiny of Figure \ref{fig:baseline_costs} is why litigation does not occur more often. In the vast majority of cases in which harm occurs, no litigation occurs as a result. There are a few cases in which the plaintiff does file a claim, and in this simulation (but not in all), the defendant answers and bargaining and a trial result. It might seem that litigation would be considerably more common. After all, if one examines Figure \ref{fig:probit}, any given level of relative precaution can correspond to eight different precaution power curves, and the probability of liability can vary greatly depending on what the actual precaution power was. 

The shading of the injury region helps provide the answer. In Figure \ref{fig:baseline_costs},  the injuries are mostly injuries for which the defendant is truly not liable (designated as "TNL Harm" in the legend). That is, the defendant did cause such injuries, but if the court knew the precaution power and applied the Hand formula, it would find the defendant not truly liable. This is so both when the defendant takes the lower level of precaution and when the defendant takes the higher level reflected in the diagram. The exceptions are cases in which the defendant received a bad signal, where the precaution power was high but the defendant received a signal that it was relatively low. 

This can also be understood by examining Figure \ref{fig:signals_at_equilibrium}, which shows the distribution of the defendant's signal given the plaintiff's at the equilibrium in Figure \ref{fig:baseline_costs}. The signals are shown conditional on the defendant's causing an injury to the plaintiff. The width of the plaintiff's signals are weighted by their frequency, and above each is a proportionately sized representation of the distribution of defendant's signals. The percentages placed in the cells sufficiently large to hold the information are the win rates that the plaintiff could expect at trial given this combination of signals. Even at a glance, one can see that the shading is relatively monotone. The plaintiff's win probability varies across only a small range, from 7.7\%, where both parties receive the lowest signal of precaution, to 19.7\%, where both receive the highest. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{../Figures/PREC010 Risk Neutral CM 1 FSM 0 DM 1 LT 1 UPC 1E-05 -winratebypdsignallight.pdf}
  \caption{Defendant's signal conditional on plaintiff's and associated plaintiff win rates, given that an accident has occurred.}
  \label{fig:signals_at_equilibrium}
\end{figure}

Figure \ref{fig:baseline_costs} also features  a small sliver of cases in which causation is wrongfully attributed to the defendant and a lawsuit results (in some cases resulting in a trial), at the very left of the right side of the diagram. This sliver is much lower than the proportion of cases in which wrongful attribution occurs, because this diagram illustrates only harms attributable to the defendant's conduct, not harms that could have occurred even if the defendant had not engaged in the underlying activity. Thus, wrongfully attributable cases are included only when they lead to litigation, because the litigation costs (along with the much lower precaution costs) are the only social cost attributable to this wrongful attribution.

The story in Figure \ref{fig:baseline_costs} is straightforward, but the baseline should be read with caution. The model is richer than standard one‑shot filing/settlement/trial frameworks—featuring endogenous disputes, two‑sided private information, and noisy adjudication—but it is not calibrated to data, and several simplifying choices matter. Bargaining is a single simultaneous‑offer round with midpoint settlement; stage costs and damages are normalized; care, signals, and offers live on discrete grids. As in Bayesian extensive‑form models generally, common knowledge of the game form and of the signal and state distributions are assumed. These features make equilibrium computation feasible at scale, but they limit external validity. Accordingly, absolute levels in Figure \ref{fig:baseline_costs} should be taken as illustrative. Our focus is comparative statics—how fee shifting, damages multipliers, or liability thresholds move total costs holding the environment fixed—where we can hope that many misspecifications cancel and the sign and relative magnitude of changes are more informative than the baseline levels.

\subsection{Three Independent Interventions}

\FloatBarrier
\paragraph{Damages multiplier.}  
Figure \ref{fig:damages_multiplier} demonstrates that social welfare is less sensitive to increases in the damages multiplier than one might anticipate. Transfers of money between plaintiff and defendant do not themselves enter the social cost accounting; what matters are changes in real expenditures on precaution, injury, and litigation. The first mechanism shaping the diagram is precaution. Higher multipliers give defendants a stronger incentive to take care, and this reduces the incidence of accidents and the costs of harm. But the precaution technology features diminishing returns: once a reasonably high level of care has been taken, further increments produce only small additional reductions in risk. The savings from fewer injuries are therefore offset by the higher precaution costs themselves, leaving total costs relatively flat through much of the multiplier range. The second mechanism is bargaining and exit. Higher potential awards do not simply push more cases into trial; cases that do enter litigation often resolve through settlement. These channels vent much of the additional pressure from increased stakes, preventing procedural expenditures from rising proportionately.  
...
This pattern aligns with and helps reconcile several strands of the literature. Rowe’s early skepticism about general predictions for American versus English rules foreshadows the modest welfare stakes of fee allocation when the fee wedge is small relative to primary behavior costs (Rowe 1984) \cite{rowe}. The sensitivity of theoretical results to settlement structure in Polinsky and Rubinfeld (1998) \cite{polinskyrubinfeld1998} also resonates here: because our bargaining protocol already resolves many disputes short of trial, fee shifting has limited room to amplify or dampen procedural spending except in corner regimes. Two‑sided information models that study fee shifting’s selection effects—such as Friedman and Wittman (2007) \cite{friedmanwittman}, Klerman, Lee, and Liu (2018) \cite{klermanleeliu}, and Dari‑Mattiacci and Saraceno (2020) \cite{darimatiaccisaraceno}—show channels that our diagrams likewise contain: weak cases exit, strong cases obtain leverage, and some parties bluff. But with the baseline magnitudes used here, those channels mostly reshuffle payoffs rather than expand real inputs.

\FloatBarrier
\paragraph{Liability threshold.}
Figure \ref{fig:liability_threshold} shows somewhat larger movements than the other interventions, though most of the welfare action still occurs in corner regimes. In our baseline costs calibration, the defendant-favorable, highest threshold (a benefit-cost threshold of 2.0) appears to minimize the sum of precaution, injury, and litigation costs. At this level, the defendant simply does not engage in precaution and, except with very low litigation costs, the plaintiff does not bother to sue. This does not mean that zero precaution is achieved; indeed, we will see later that the social optimum does involve positive precaution. But it does suggest the possibility that from a pure efficiency perspective, given the costs and practical limitations of litigation, it sometimes may be more efficient for society to bear the costs than to allow litigation. Still, this point should not be casually generalized beyond the model explored here. The middle range of thresholds ($0.8$, $1.0$, and $1.2$) all produce relatively similar results, with higher levels slightly reducing social cost.
...
The diagrams do suggest that strict liability (corresponding to a threshold of $0$, so that the defendant is always liable regardless of the relative precaution level chosen) can perform relatively poorly once litigation costs are included in this model. The reason turns on who pays costs first. In the negligence panels, the vast majority of injury is “truly not liable” harm for which plaintiffs rationally do not file; fault must be shown, so many potential disputes are screened out at the filing stage. Under strict liability, by contrast, all such harms become compensable regardless of the defendant’s care, and so all of the injury is of the "truly liable" variety. Given our move order, plaintiffs bear the filing cost up front while defendants frequently default rather than answer, so \emph{plaintiff} litigation outlays dominate in the strict-liability rows even though defendants’ own outlays remain small. From a defendant’s perspective, litigation costs are largely external and liability is essentially inevitable, so cutting precaution can be privately sensible even when it increases harm at the margin. 

This asymmetric cost bearing aligns with well-known comparative statics: negligence regimes tend to generate fewer suits because fault must be established, whereas strict liability invites filing after any harmful outcome (e.g., Shavell 2003 \cite{shavell2003}). At the same time, uncertainty about the negligence boundary gives precaution a second private payoff—reducing the probability of being held liable at all—so care is often higher under negligence than under strict liability (Calfee and Craswell 1984) \cite{calfeecraswell}. Put together, the diagrams’ pattern follows: at very low costs, strict liability can be slightly more efficient because the filing burden is cheap and broader compensation modestly improves incentives; as costs rise, the plaintiff-first filing asymmetry under strict liability swamps those gains, while negligence’s screening conserves resources even if it tolerates somewhat more injury. In a more realistic setting, defendants might agree to pre-suit resolutions that would avoid the need for plaintiffs to file at all, strengthening the case for strict liability.
...
Figure \ref{fig:dm_fee} shows that these two levers mostly operate on different, weakly connected margins, so their joint effect is close to additive and small. "Low D" and "High D" correspond to damages multipliers of 0.5 and 2, respectively. "Low F" and "High F" correspond to fee shifting multipliers of 0 and 2, respectively. The damages multiplier works, if at all, by altering precaution incentives and case selection; fee shifting reassigns relatively modest stage costs and nudges bargaining posture. Increasing the fee-shifting multiple barely changes real resource use. The relatively modest effects indicate that, controlling for the level of damages multiplier, double fee shifting is very modestly beneficial (though there is no effect with the low damages multiplier at the highest cost level). At either level of fee shifting, switching from the low to the higher damages multiplier is modestly harmful, at least at baseline or low litigation costs. But high damages multipliers greatly outperform low damages multipliers at the highest cost levels, consistent with Figure \ref{fig:damages_multiplier}.

\paragraph{Damages multiplier and liability threshold.}
Figure \ref{fig:dm_liability} shows that the liability threshold drives larger differences in overall social costs than the damages multiplier, and the two levers interact mostly by dampening one another rather than compounding. "Low L" refers to a liability threshold of 0.8, and "High L" refers to a threshold of 1.2. Within any fixed liability threshold, moving from a low to a high damages multiplier mainly shifts composition: higher precaution, fewer realized harms, modestly different bargaining and exit patterns. The total bar, however, remains nearly constant. An exception is that the plaintiff-friendly combination of a high damages multiple with a low liability threshold increases costs at baseline and with low litigation costs (but performs well at high litigation costs). By contrast, tightening the liability threshold from plaintiff-friendly to defendant-friendly screens out many potential claims, cutting filing and procedural expenditures regardless of the damages setting. Once that screen is in place, raising damages does little, because liability attaches so rarely.
...
Figure \ref{fig:fee_liability} tells a similar story: Overall, the social costs level is relatively static, with higher liability thresholds producing modest savings controlling for fee-shifting level. Figure \ref{fig:fee_liability} does feature the panel that has the strongest performance at baseline costs of any intervention seen so far: the combination of high fee-shifting with a high liability threshold. This is optimal not because it leads to settlements, but because plaintiffs are hesitant to file in truly not liable cases and even in cases in the few cases in which the defendant is truly liable even given the higher liability threshold. The defendant does engage in at least the most essential precaution, providing an advantage over panels in which the defendant does not engage in precaution at all. The legal system thus avoids the worst behavior while also avoiding costly litigation.
...
This section stress‑tests the main findings by varying the informational, behavioral, and institutional ingredients that shape equilibration. We begin by tricking defendants to misestimate the cost of care, which disperses precaution and thickens the region near the negligence boundary. We then introduce risk aversion and trace how aversion reallocates resources from harm to precaution while pulling disputes forward into settlement. Next, we alter the unit cost of precaution to see when cheaper prevention amplifies or dampens the effects of damages. We consider information asymmetries and asymmetric litigation costs to examine how private knowledge and budget imbalances bend filing and bargaining margins. We raise and lower the probability that harm is wrongfully attributed to the defendant to study selection into litigation when causation is noisy. Finally, we vary court quality—both evidentiary noise and dispersion in judicial thresholds—to ask when adjudication sharpens incentives or, paradoxically, blurs them enough to reduce waste. Other than in the misestimation checks, we keep the modeling architecture constant so that movements in the figures can be read as consequences of the perturbation rather than artifacts of a new game.

\FloatBarrier
\subsection{Imperfect Estimation}

A central worry about the baseline model is that the clean, strong equilibrium patterns may be partly an artifact of assuming complete information at the care stage. Real defendants rarely have a crisp, shared view of the trade-off the negligence test asks them to make. They misread internal budgets, discount or overstate hassle costs, and—perhaps most likely—misjudge the other side of the ledger: what the magnitude of injury might be. Because the court’s rule and the defendant’s decision both hinge on a comparison of “expected harm avoided” to “cost of precaution,” any broad uncertainty about the magnitude of injury can be represented, without loss of generality, as uncertainty about the effective price of care.

We therefore introduce a coarse misestimation device that is intentionally not a small Gaussian wobble but a substantial, case‑specific shift in the defendant’s private optimization. At the moment the defendant chooses precaution, a chance draw sets the perceived step cost to one of three levels—much lower than true, accurate, or much higher—each equally likely. In the running example, a misestimation level of five means the defendant feels the step as one‑fifth, exactly, or five times the true cost. To keep the extensive form game tree a reasonable size, we reduce the number of precaution power, relative precaution, signal and offer levels to 5 from 8; the size of the game tree becomes 109,609 nodes. Nothing else in the world moves: the technology stays fixed, the court continues to apply the negligence threshold using the true cost, and welfare accounting uses the true cost rather than the misestimation. Only the defendant’s private calculus is distorted, and the draw is not observed by the plaintiff or the court. 

The advantage of focusing the misestimation on care rather than injury is that this approach does not distort the litigation process. We can still compare to other simulations, because the normalized cost of injury remains at $1$. But analytically, adding misestimation to cost estimation is equivalent to adding misestimation to injury. What drives the model is the relative size of precaution costs and injury. And so the analysis here is broadly applicable to a model in which the defendant is certain of costs but highly uncertain of the magnitude of injuries that might result. The point is to stress‑test conclusions that rely on precise private optimization: if those conclusions are fragile to coarse miscalibration, then the baseline results should be read more cautiously.

Figure \ref{fig:imperfect_estimation} shows what happens as the misestimation level rises, focusing only on the baseline level of costs. Two patterns are immediate. First, the precaution bar becomes visibly more heterogeneous. Some defendants, feeling care is cheap, climb to higher levels; others, feeling it is dear, scale back. The averages move only a little, but the spread grows. Second, the system gets a bit busier procedurally. With more mass near the noisy negligence boundary, plaintiffs file slightly more often, defendants answer more often, bargaining runs more frequently, and trials tick up modestly. These are composition effects rather than explosions in inputs: Greater care by some defendants partly offsets lower care by others, so the total bar remains fairly flat through the middle of the range. What changes is the mix—more dispersion in primary behavior and a thicker layer of litigation activity.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Cost Breakdown Misestimation Level (Single Row).pdf}
  \caption{Social costs under imperfect estimation of precaution cost.}
  \label{fig:imperfect_estimation}
\end{figure}

When we vary damages against this backdrop, as with the Figure \ref{fig:imperfect_estimation_damages} panels featuring misestimation levels of 5, parts of the qualitative picture remains familiar. Higher damages amplify incentives for those who already perceive care as cheap and soften under‑investment among those who perceive it as expensive, which narrows the dispersion in care somewhat. At the same time, larger stakes make early exits slightly more attractive for weak positions, shifting some mass from bargaining and trial into earlier resolution.  Yet overall, changing damages multipliers has a much more substantial effect on social welfare than in the baseline simulation. The total social cost rises from $1.20 \times 10^{-4}$ with a damages multiplier of $1$ to $1.45 \times 10^{-4}$ with a multiplier of $2$. And a damages multiplier of $\frac{1}{2}$ lowers total social costs to $1.10 \times 10^{-4}$. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Cost Breakdown Damages Multiplier (High Misestimation) (All Rows).pdf}
  \caption{Social costs under high misestimation of precaution cost.}
  \label{fig:imperfect_estimation_damages}
\end{figure}

We must be careful, however, about making direct comparisons to our baseline simulations, given the change in the number of precaution power, relative precaution, signal, and offer levels. Figure \ref{fig:imperfect_estimation_damages_graph} offers a more apt comparison, focusing on the various components in social welfare level in comparably sized graphs at the baseline litigation costs level. These graphs confirm that as misestimation levels become high, the social welfare cost line becomes steeper, as the slight decline in harm cost is insufficient to offset the increases in precaution cost and litigation expenditures.  

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Social Welfare Loss Varying Misestimation Level.pdf}
  \caption{Social costs under various misestimation levels.}
  \label{fig:imperfect_estimation_damages_graph}
\end{figure}

Earlier, we observed social welfare stay relatively constant as a result of the legal system equilibrating, but such equilibration is more difficult in an environment in which parties make systematic misestimates. And the optimal policy may well depend on the direction of the misestimation. If one suspected that defendants systematically underestimate the probability of injury, relatively high damages multipliers would likely be an appropriate policy response.

\FloatBarrier
\subsection{Risk Aversion}

Risk aversion alters the equilibrium in two channels at once. First, it increases the private value of avoiding downside risk, which nudges defendants toward somewhat higher levels of precaution. Second, it reshapes bargaining posture: both plaintiffs and defendants prefer to avoid trial, leading more disputes to resolve at earlier stages. These forces largely offset in aggregate, so the overall comparative statics look familiar, though the composition of costs shifts.

Figure \ref{fig:risk_aversion} displays the results across damages multipliers and cost environments. Two patterns stand out. The first is a reallocation from harm into precaution. Defendants pay a modest premium to avoid the tail risk of trial outcomes, and as a result, accidents decline even without changes to legal doctrine. The second is a reduction in trials. Cases that would otherwise have proceeded further under risk neutrality now exit earlier, either through settlement or default. The proportion of matters that ever reach trial is already low; with risk aversion, it becomes lower still.
...
Across damages multipliers, total social costs remain nearly constant through much of the range. Higher multipliers strengthen defendants’ incentives to take care and plaintiffs’ willingness to bank value in settlement; lower multipliers relax both tendencies. Because transfers do not themselves enter the welfare calculus, the additional savings from reduced injury are largely absorbed by increased precaution spending, leaving overall costs steady except at the extremes. When litigation is especially costly, very low damages perform poorly: defendants under-invest in care, while plaintiffs still pay filing costs to pursue cases, pushing total costs upward. At the opposite end, very high damages do not trigger a wave of trials; rather, they lead to greater precaution and earlier exits, again keeping total costs from rising sharply.

A companion perspective comes from Figure \ref{fig:risk_asymmetry}, which varies \emph{who} is risk averse. When plaintiffs alone are risk averse, the injury cost series falls while the procedural–expenditure series stays small but persistent—plaintiffs still file to create bargaining opportunities, yet shy away from trial risk and settle earlier. When defendants alone are risk averse, the precaution cost series rises modestly and the injury cost series falls: defendants pay a small premium in prevention (and, at the margin, in activity choices) to trim downside tails, which lowers realized harm. With stronger asymmetry—plaintiffs much more risk averse or defendants much more risk averse—those same patterns are sharper.

Two implications follow. First, asymmetry mainly reshuffles where resources are spent—toward prevention under defendant-side risk aversion, toward front-end filing and bargaining under plaintiff-side risk aversion—without producing large movements in the red aggregate line. Second, varying the damages multiplier tilts composition more than it moves totals: higher damages amplify whichever margin already matters (precaution when defendants are risk averse; early settlement when plaintiffs are), yet the aggregate remains buffered because private adjustments in care and bargaining absorb most of the pressure.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Social Welfare Loss Varying Risk Aversion Asymmetry.pdf}
  \caption{Social welfare loss under risk-aversion asymmetry across damages multipliers.}
  \label{fig:risk_asymmetry}
\end{figure}

\FloatBarrier
\subsection{Cost of Precaution}

Figure \ref{fig:cost_precaution} shows how social welfare loss responds when the unit cost of precaution varies. Normal unit cost remains $1.0 \times 10^{-5}$. The other three levels tested ("Very Low", "Low" and "High") are $1.0 \times 10^{-6}$, $0.8 \times 10^{-5}$, and $1.2 \times 10^{-5}$. Cheaper precaution is, of course, a boon, as the cost of precaution is itself a component of the welfare analysis, and potential defendants will take more care will relatively low precaution costs. Yet at the baseline and inexpensive cost levels, the core pattern that we have observed of damages multipliers having relatively minimal consequence for social welfare remains intact. Damages multipliers increase relative precaution costs even at high levels of precaution costs, and this increased expenditure reduces injury and has modest effects on litigation. Overall, social costs rise slightly with high damages multipliers and fall slightly with reduced damages, as in our baseline simulations. 

Once again, very expensive litigation produces a different patern. Here, at very low unit precaution cost, low damages multipliers are preferable, as precaution levels are essentially maxed out and result in more litigation but no decrease in harm. At other levels of precaution cost, low damages multipliers produce greater social cost, as the defendant engages in too little precaution. There is virtually no difference, however, in total social welfare between damages multipliers of $1$ and $2$. 

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Social Welfare Loss Varying Unit Precaution Cost.pdf}
  \caption{Social welfare loss under changes in cost of precaution.}
...
We vary which party’s private signal about liability is more informative while holding the court’s signal fixed and all other primitives at their baseline.

\begin{center}
  \includegraphics[width=0.85\textwidth]{../Figures/Cost Breakdown Information Asymmetry (All Rows).pdf}
  \captionof{figure}{Cost breakdown under information asymmetry.}
  \label{fig:info_asymmetry}
\end{center}

In Figure \ref{fig:info_asymmetry}, the party with the better information has half the noise multiplier that it would ordinarily have, while its opponent has twice the noise multiplier. The figure shows that total social cost is quite stable across regimes; the main effects are again compositional. When defendants are better informed, procedural activity expands (more answering and bargaining) and precaution softens. When plaintiffs are better informed, filing is more selective, settlements arrive earlier, and precaution rises slightly. The cost environment moderates these patterns: with cheap procedure, defendant‑side informational advantages tilt behavior toward additional procedural consumption; with expensive procedure, the same advantages curb contests and modestly reduce procedural outlays. In all cases, the swings in totals are small relative to the baseline. Though not shown here, changing the damages multiplier also has muted effects on total social welfare loss at baseline and low levels of costs. 


\FloatBarrier
\subsection{Costs Asymmetry}

Creating asymmetry in litigation costs also has little implication for the optimal damages multiplier, though there are slightly more pronounced effects on social welfare of the asymmetry, without changing from the baseline damages multiplier. In Figure \ref{fig:costs_asymmetry}, in the "P Lower Costs" column, the plaintiff's litigation costs are halved relative to the normal level for that row; in the "P Higher Costs" column, they are doubled. When the plaintiff enjoys a litigation cost advantage, the defendant at any given level of overall costs will engage in a greater degree of precaution. Litigation will also be more common, except at the highest levels of overall costs, where the defendant would recognize in any settlement negotiations that the plaintiff would not have a credible threat to take a case to trial. As we have seen with earlier changes to the model, the plaintiff-friendly change here tends to increase total social costs but only slightly. The regime with the lowest total social cost, at all three levels of overall litigation costs, is the regime in which the plaintiff bears relatively higher costs. 
...
A change in the wrongful attribution probability can have a significant effect on welfare, though the effect is more substantial when litigation costs are relatively low. In Figure \ref{fig:wrongful_attr} disaggregates social costs, varying the probability that a defendant who has not caused an injury to the plaintiff will wrongfully be thought to have done so. Recall that in such cases, the causation issue is assumed to be settled, and the issue remains whether the defendant exercised sufficient care. We can see that with high wrongful attribution probability (10 times the usual level), litigation becomes considerably more common. But that effect is relatively insubstantial at the baseline level of costs, since plaintiffs still will not sue when the plaintiff's signal indicates that the defendant has likely exercised due care. With very low litigation costs, however, it is worth it for the plaintiff to file and to extract a nuisance settlement in some cases, with other cases going to trial. This considerably increases litigation costs. 
...
The unsurprising implication is that in this condition, dividing damages in half will improve social welfare, as indicated in Figure \ref{fig:wrongful_attr_damages_mult}. But such a low damages multiplier is especially problematic in a high litigation cost environment, leading to the undesirable combination of suboptimal precaution and litigation in the form of plaintiffs' filing to claim their damages.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Social Welfare Loss Varying Wrongful Attribution Probability.pdf}
  \caption{Social welfare loss under changes in the probability of wrongful attribution of liability.}
  \label{fig:wrongful_attr_damages_mult}
\end{figure}

\FloatBarrier
\subsection{Court Quality}

Finally, let us consider court quality. Recall that courts may be considered imperfect in two ways. First, a court, like the parties, receives only a signal of the precaution power. Second, each individual judge may have an idiosyncratic threshold at which to apply liability. In Figure \ref{fig:court_quality}, with "Low Noise," the noise signal is halved and with "High Noise," it is doubled. "Low Indet" refers to a state in which there is less indeterminacy (the variable is set to $0.1$ instead of $0.25$), and "High Indet" refers to a state in which there is high indeterminacy ($0.5$). Once again, social welfare within each row is relatively consistent, though there are significant changes in composition. At baseline costs, with higher indeterminacy, the defendant's relative precaution level becomes homogeneous. As the legal standard becomes fuzzier, there is less reason to tailor to the defendant's signal of precaution power, but the defendant becomes somewhat more cautious (causing greater social loss) overall. Interestingly, high court noise also leads to more homogeneous action by the defendant, but at a lower total level with baseline and high costs.
...
An interesting aspect of Figure \ref{fig:court_quality} is that the social welfare under a perfect system of adjudication is lower than in some other systems. This highlights that the complex interactions in the endogenous model can lead to counterintuitive results. But this is in part because the "perfect" adjudication in that figure means only that the courts are perfect, not that the litigants are. Figure \ref{fig:perfect} compares the baseline to a regime in which the parties are fully informed and litigation is costless. Litigation occurs in every case resulting in injury in this regime, but bears no costs. Meanwhile, the the defendant's choice of relative precaution level increases gradually based on the defendant's signals, and the social welfare loss is considerably lower than at baseline, $9.90 \times 10^{-5}$ versus $1.04 \times 10^-4$. This provides a target for maximal efficiency that can be used to assess procedural mechanisms. At the same time, it reflects that in an equilibrating legal system, the difference between baseline and perfect may not be all that great. That helps explain why it is hard for procedural interventions to make things much better, though possible for a litigation system with very high costs and mediocre results to make things considerably worse.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{../Figures/Cost Breakdown Adjudication Mode (Single Row).pdf}
  \caption{Cost breakdown with no noise for parties or court and no indeterminacy.}
  \label{fig:perfect}
\end{figure}

\section{Conclusion}

This article has argued that once disputes arise endogenously from care choices and are resolved under two‑sided uncertainty, canonical procedural devices have less bite than their caricatures suggest. The damages multiplier changes the slope of private payoffs but also moves bargaining, quitting, and precaution in offsetting directions; fee shifting mostly reassigns modest stage costs; and even liability thresholds, though more consequential, shift totals only gradually across a broad middle range. The picture that emerges is hydraulic. Incentives introduced at one stage are absorbed by adjustments at others, leaving composition to do most of the visible work while aggregate social cost drifts only at the edges. 

That conclusion should be read with appropriate humility. It does not say that procedure is neutral, only that within a realistically responsive system, levers that operate through transfers or through single margins encounter endogenous counter‑movements. The model also exposes pockets where equilibrating forces are weaker—very high litigation costs, very plaintiff‑favorable standards, or environments in which parties systematically misestimate the trade-off between precaution and harm. In those pockets, damages and thresholds interact in ways that produce meaningful welfare differences.

Two lines of further work seem especially valuable. One is empirical calibration. Because the architecture already nests filing, answering, simultaneous offers, contingent quitting, and trial under noisy signals, a structural fit to data at the industry or claims‑category level could discipline parameter choices and test whether the predicted dampening of policy effects appears in the wild. A second line concerns institutional granularity: richer bargaining protocols, repeat‑play and reputation effects, third‑party finance, and multi‑plaintiff aggregation would let the model speak more directly to modern litigation markets and to reforms that change who funds, bundles, or sequences claims.

The most promising next step, however, will be to revisit punitive or enhanced damages with a design that is variable rather than fixed. The classical calibration sets damages at harm divided by the probability of detection. In practice, detection is neither known nor constant: actors face uncertainty about the level of detection in a given context and heterogeneity across contexts in the distribution of detection risk. Building those features into the present framework would permit multipliers that respond to both uncertain detection and variability in the probability of detection itself. Technically, this means treating the detection probability as a hidden state alongside precaution power, allowing litigants and courts to receive noisy signals about its mean and dispersion, and then comparing regimes in which the multiplier is fixed ex ante to regimes in which it is case‑responsive—either through explicit menus keyed to observables or through posterior‑contingent awards. Such a model would surface trade-offs that fixed multipliers cannot capture: the value of robustness when detection is poorly known, the risk of overcorrection when detection is merely volatile, and the strategic feedbacks when parties infer multipliers from enforcement patterns.
...
Nature draws a hidden precaution–power state $\theta \in \Theta = \{1,\dots,n_H\}$ uniformly. Each such state determines the center of a precaution–power distribution rather than a deterministic value. We index $\theta$ to the unit interval by
\[
\varphi(\theta) \;=\; \frac{\theta}{\,n_H+1\,},
\]
so that $\varphi(\theta)$ always lies in $(0,1)$. Linear interpolation between endpoint benchmarks at $\varphi=0$ and $\varphi=1$ determines parameter values for accident probabilities.

Conditional on $\theta$, the plaintiff and defendant each observe a private signal about precaution power. Signals are generated by partitioning the unit interval into equal bins and then adding truncated normal noise. For the plaintiff, define midpoints
\[
SP_j \;=\; \frac{j-\tfrac{1}{2}}{n_{S_P}}, \qquad j=1,\dots,n_{S_P},
\]
and let $\sigma_{P}$ be the plaintiff’s noise parameter. The conditional distribution is
\[
\Pr(s_P=j \mid \theta)
=
\frac{
\Phi\!\left(\dfrac{SP_j+\tfrac{1}{2n_{S_P}}-\varphi(\theta)}{\sigma_{P}}\right)
-
\Phi\!\left(\dfrac{SP_j-\tfrac{1}{2n_{S_P}}-\varphi(\theta)}{\sigma_{P}}\right)
}{
\Phi\!\left(\dfrac{1-\varphi(\theta)}{\sigma_{P}}\right)
-
\Phi\!\left(\dfrac{-\varphi(\theta)}{\sigma_{P}}\right)
},
\]
where $\Phi$ is the standard normal cdf. 

For the defendant, define analogously
\[
SD_j \;=\; \frac{j-\tfrac{1}{2}}{n_{S_D}}, \qquad j=1,\dots,n_{S_D},
\]
with noise parameter $\sigma_{D}$, and replace $\sigma_{P}$ by $\sigma_{D}$ in the expression above. Signals are conditionally independent given $\theta$, but positively correlated in the unconditional distribution through their shared dependence on the hidden state. The unconditional law of each signal is obtained by averaging the conditional probabilities across the uniform prior on $\theta$.

Unless noted otherwise, we use equal noise levels across parties, setting
\[
\sigma_P = \sigma_D = 0.2
\]
on the $[0,1]$ latent scale.


\paragraph{Accident probability.}

The defendant then decides whether to engage in the activity, which avoids a baseline opportunity cost of $\beta = 1.0 \times 10^{-3}$ if not undertaken, and, if the defendant engages, chooses a relative precaution level $k \in K = \{0,\dots,n_K-1\}$. Note that the lowest level here corresponds to no precaution undertaken at all. We index the relative precaution level $k$ on $[0,1)$ via $\tau(k) \;=\; \frac{k}{n_K }$.
...
Note that while the share of wrongful cases rises with care, the level of attributed accidents decreases in care, assuming $0 < \rho < 1$. Let $A\in\{0,1\}$ denote the legally attributed-accident indicator, with $\Pr(A=1\mid \theta,k)=p_{\mathrm{acc}}(\theta,k)$.

Although defendants choose from the discrete grid $k\in\{0,\dots,n_K-1\}$ with $\tau_k=k/n_K$, the court may apply the continuous marginal test or one of two discrete “next‑step” variants: a safe‑harbor option (defining the top increment as $0$) or a phantom‑level option (evaluating the next‑step difference even at the top by positing $k+1$). As $n_K$ increases, the discrete grid approximates the continuous ideal, and results under all three conventions converge as the action space becomes finer.

\paragraph{Filing and answering.}
Following an accident realization, the plaintiff, who has observed $s_P$, chooses whether to \emph{file}, incurring $c_{file}=0.10$ if she does so. If the plaintiff files, the defendant observes $s_D$ and chooses whether to \emph{answer}. Answering imposes $c_{answer}=0.10$ and moves the case into bargaining. 
...
\[
T \;=\; \mu \times \frac{\mathcal{o}_P+\mathcal{o}_D}{2}\in[0,\mu],
\]
and the case terminates without trial. Although damages multipliers apply to settlements (which are conceived as being negotiated between $0$ and $1$), fee-shifting does not. No Rule 68 or offer–of–judgment mechanism is considered here. 

\paragraph{Exits.}
If the offers do not meet, the game enters the post-offer exit stage: the plaintiff and defendant then simultaneously choose whether to abandon or default; if both quit, a symmetric chance node selects which quit occurs; if neither quits, the case proceeds to trial.
...
\]
with distribution conditioned on the hidden precaution–power state $\theta$ by the same truncated–normal mechanism used for party signals. Let $SC_j=\dfrac{j-\tfrac12}{n_{S_C}}$ and let $\sigma_{C}>0$ denote the court’s noise parameter. Then
\[
\Pr(s_C=j\mid \theta)
=
\frac{
\Phi\!\left(\dfrac{SC_j+\tfrac{1}{2n_{S_C}}-\varphi(\theta)}{\sigma_{C}}\right)
-
\Phi\!\left(\dfrac{SC_j-\tfrac{1}{2n_{S_C}}-\varphi(\theta)}{\sigma_{C}}\right)
}{
\Phi\!\left(\dfrac{1-\varphi(\theta)}{\sigma_{C}}\right)
-
\Phi\!\left(\dfrac{-\varphi(\theta)}{\sigma_{C}}\right)
},
...
The court's task is to compare the marginal cost of precaution with the marginal benefit. The scale parameter $s$ in $w(c,k)=\Phi((R_\star(c,k)-\lambda)/s)$ captures dispersion in judges’ personal liability thresholds around the official $\lambda$. It is distinct from $\sigma_C$, which governs evidentiary noise in the court’s signal of precaution power.

\paragraph{Alternative conventions for marginal benefit calculation.}
The code supports three conventions for evaluating the marginal reduction in accident risk at a precaution level \(\tau_k\). Each has advantages and drawbacks, both as a model of real precaution decisions and as a computational device. From the behavioral perspective, some contexts plausibly involve continuous precaution choices (e.g., investment in maintenance effort or the level of monitoring intensity), while in many legal–institutional settings defendants face only finitely many meaningful care levels (e.g., whether to install a guardrail or not). From the modeling perspective, the continuous approach is analytically smooth and aligns with solvers that assume fine granularity, whereas the discrete approaches align exactly with a finite action space and permit cleaner welfare comparisons when care levels are indivisible.

\medskip
\noindent\emph{Continuous-derivative convention.}
...
\medskip
\noindent\emph{Discrete “next-step” conventions.}
When precaution is represented by a finite grid of levels, one can define the marginal benefit of the next step in more than one way:
\[
...
\end{cases}
\]
At interior levels, this expression is nonnegative because accident risk decreases with precaution, and the wrongful-attribution term cancels just as in the continuous case. At the top level, however, two interpretations are possible. Under the \emph{safe-harbor convention}, the final increment is defined as \(0\), so the highest level of precaution automatically satisfies the negligence test. This corresponds to a legal regime in which the most precautionary observable act provides full insulation from liability. Under the \emph{phantom-level convention}, by contrast, the formula applies even at the top step by positing a hypothetical further increment \(k+1\). This reflects the idea that courts may consider broader hypothetical options even when the defendant has reached the top of a discretized scale, and that a categorical safe harbor may be unrealistic in litigation, particularly given the safe-harbor treatment discussed elsewhere in the model. In our baseline simulations we use the phantom-level convention. This represents a compromise while also being more compatible with the probit-style distributed-threshold rule discussed below in the liability-rule subsection. Because the probit function $\Phi((R_\star-\lambda)/s)$ presumes a continuum of possible marginal-benefit ratios, treating the top precaution level as if a further increment $k+1$ were available preserves smooth variation in $R_\star$; by contrast, a safe-harbor convention would force $R_\star=0$ at the boundary, creating a discontinuity inconsistent with the probit specification.

With \(\bar{\Delta}^{\mathrm{disc}}(c,k)\) denoting the posterior expectation under whichever discrete convention is chosen, the court compares the expected discrete benefit of the next step to the \emph{cost of one step}:
\[
...
Having specified the marginal-benefit convention—continuous derivative, discrete safe harbor, or discrete phantom—we now turn to how the court evaluates the expected benefit of an additional precaution, conditional on its signal. Given a realized court signal $s_C=c$, Bayes’ rule with the uniform prior $\Pr(\theta)=1/n_H$ gives the posterior distribution $\Pr(\theta \mid s_C=c)$. The expected marginal reduction at $(c,k)$ is
\[
\bar{\Delta}(c,k)\;=\;\sum_{\theta\in\Theta}\Delta^{*}(\theta,\tau_k)\,\Pr(\theta\mid s_C=c),
\]
where $\Delta^{*}$ denotes the convention in use (continuous derivative, discrete safe-harbor difference, or discrete phantom difference). The posterior $\Pr(\theta\mid s_C=c)$ is formed from the ex ante prior (i.e., it does not condition on $A$); in particular, we ignore case-selection effects when computing $\bar{\Delta}$.
...
R_{\mathrm{cont}}(c,k)=\frac{\bar{\Delta}(c,k)}{\,n_K\,c_{\mathrm{prec}}\,},
\]
where $n_K c_{\mathrm{prec}}$ is the cost per unit $\tau$. Under either discrete convention (benefit of the next step, defined with or without a phantom level), the court compares
\[
...
\medskip
\noindent\textbf{Liability rule (deterministic vs.\ distributed thresholds).}
Given $R_\star(c,k)$, the court applies one of two liability rules. In the \emph{deterministic threshold} rule, all judges are assumed to apply the same official liability threshold $\lambda$, so the verdict is
\[
V=\mathbf{1}\!\{R_\star(c,k)>\lambda\}.
\]
In the \emph{distributed-threshold} rule, we model each judge as having a personal cutoff $\lambda_j$ equal to the official threshold $\lambda$ plus a random deviation. Specifically, we assume
\[
\lambda_j = \lambda + sZ, \quad Z\sim N(0,1),
\]
so that judges differ in how much higher or lower a benefit–cost ratio they require relative to the doctrinal benchmark. For a given $(c,k)$, the probability of liability is then the fraction of judges whose cutoff lies below $R_\star(c,k)$:
\[
w(c,k)=\Pr(\lambda_j \leq R_\star(c,k))=\Phi\!\Bigl(\tfrac{R_\star(c,k)-\lambda}{s}\Bigr).
\]
The verdict is then drawn $V\sim \mathrm{Bernoulli}(w(c,k))$. Unless otherwise noted, we set $\lambda=1.0$ and use the distributed-threshold rule with $s=0.25$. Alternative $\lambda$ and $s$ values, including $s=0$ to reflect a deterministic rule, also examined.

\noindent\textbf{Probability of liability at trial.}
Conditional on hidden state $\theta$ and chosen precaution level $k$, the probability that the court returns a liability verdict integrates over the court’s noisy signal:
\[
\Pr(V=1 \mid \theta,k)\;=\;\sum_{c\in\mathcal{S}_C} w(c,k)\,\Pr(s_C=c\mid \theta),
\qquad
\Pr(V=0 \mid \theta,k)\;=\;\sum_{c\in\mathcal{S}_C} \bigl(1-w(c,k)\bigr)\,\Pr(s_C=c\mid \theta).
\]
...
\]
No fee–shifting applies to settlements.
...
\emph{Default and Defendant quit.} If the defendant defaults at the post-offer exit stage, the plaintiff receives a judgment of $\mu$, reflecting the damages multiplier, and fee-shifting does not apply. Final wealths are
\[
W_P^{\mathrm{final}} \;=\; W_P^{(0)} - Z - C_P + \mu,
\qquad
W_D^{\mathrm{final}} \;=\; W_D^{(0)} - k\,c_{\mathrm{prec}} - C_D - \mu,
\]
where $C_P$ and $C_D$ include the stage costs incurred up to that point (filing for the plaintiff; answering for the defendant; and bargaining for both), but not trial costs.

\emph{Abandon.} If the plaintiff abandons at the post-offer exit stage, no judgment is entered. Neither damages multipliers nor fee-shifting apply. Final wealths are
\[
W_P^{\mathrm{final}} \;=\; W_P^{(0)} - Z - C_P,
\qquad
W_D^{\mathrm{final}} \;=\; W_D^{(0)} - k\,c_{\mathrm{prec}} - C_D,
\]
where $C_P$ and $C_D$ include all stage costs already incurred along the realized path (filing and answering where applicable, and bargaining), but not trial costs.
...
Explicitly modeling every chance node (random event) can cause an explosion in the size of the game tree. We thus use Bayesian marginalization of chance nodes to avoid explicitly branching on a random variable at the outset of the game. Instead, we integrate out uncertainties concerning the variable represented by the node on the fly, adjusting probabilities and outcomes accordingly. This approach keeps the tree smaller without changing the underlying game. Specifically, given the game settings (excluding the robustness checks with manipulation), this keeps the extensive form game tree at 225,497 nodes, instead of at 1,290,752 nodes. This not only results in a proportional speed up of the algorithm but also reduces memory requirements proportionately. Because memory is the limiting factor in our ability to process game trees in parallel, even when running on a machine with 128 GB of RAM, the Bayesian marginalization makes it possible to process more tree permutations at once and to process them faster, thus allowing a speedup of approximately 30 times, though this will be platform-specific.

In our litigation game, the first chance event determines the precaution power state: the hidden state of which the defendant, plaintiff, and court may eventually receive signals. Instead of selecting such a state, we can convert the game into a mathematically equivalent game. The strategic players' information sets in this game are exactly the same as those in the original game, because the hidden state never becomes part of the plaintiff's or defendant's information set. 
...
For each $(c,k)$ pair, let $w(c,k)\in[0,1]$ denote the court’s decision weight (Appendix~A). Under the deterministic threshold, $w(c,k)=\mathbf{1}\{R_\star(c,k)>\lambda\}$; under the distributed-threshold rule, $w(c,k)=\Phi\!\bigl((R_\star(c,k)-\lambda)/s\bigr)$.

For implementation efficiency and to avoid selection effects, we start from the prior court-signal slice $\Pr(s_C\mid p,d)$ supplied by the signal model and obtain the verdict-conditioned distributions by reweighting and renormalizing:
\[
\Pr(s_C=c\mid p,d,k,\text{liable}) \;\propto\; w(c,k)\,\Pr(s_C=c\mid p,d),\qquad
\Pr(s_C=c\mid p,d,k,\text{no liable}) \;\propto\; \bigl(1-w(c,k)\bigr)\,\Pr(s_C=c\mid p,d).
\]
The probability of a liability verdict in the collapsed tree is given by the posterior-weighted mixture over hidden states:
\begin{align*}
\Pr(V=1 \mid s_D, A, k, s_P)
  &= \sum_{\theta\in\Theta} 
       \Pr(\theta \mid s_D, A, k, s_P)
       \sum_{c\in\mathcal{S}_C} 
          w(c,k)\,\Pr(s_C=c\mid \theta), \\[6pt]
\Pr(V=0 \mid s_D, A, k, s_P)
  &= \sum_{\theta\in\Theta} 
       \Pr(\theta \mid s_D, A, k, s_P)
       \sum_{c\in\mathcal{S}_C} 
          \bigl(1-w(c,k)\bigr)\,\Pr(s_C=c\mid \theta).
\end{align*}
...
= \frac{\Bigl(\sum_{c} w(c,k)\,\Pr(s_C=c\mid \theta)\Bigr)\,
       \Pr(\theta \mid s_D, A, k, s_P)}
       {\sum_{\theta'\in\Theta}\Bigl(\sum_{c} w(c,k)\,\Pr(s_C=c\mid \theta')\Bigr)\,
        \Pr(\theta' \mid s_D, A, k, s_P)},
\]
and for a no‑liability verdict,
\[
\Pr(\theta \mid s_D, A, k, s_P, V=0)
= \frac{\Bigl(\sum_{c} \bigl(1-w(c,k)\bigr)\,\Pr(s_C=c\mid \theta)\Bigr)\,
       \Pr(\theta \mid s_D, A, k, s_P)}
       {\sum_{\theta'\in\Theta}\Bigl(\sum_{c} \bigl(1-w(c,k)\bigr)\,\Pr(s_C=c\mid \theta')\Bigr)\,
        \Pr(\theta' \mid s_D, A, k, s_P)}.
\]
...
\cite{lanctot}. For each player $\phi$, let 
...
method of Johanson et al. (2011) \cite{johanson}. This procedure calculates, in a forward pass, 
...
Because the scale of payoffs varies, we rescale each player’s performance 
affinely to the unit interval, with $0$ corresponding to the lowest attainable 
payoff in the game tree and $1$ to the highest. Exploitability values are thus 
reported on this $[0,1]$ scale.
